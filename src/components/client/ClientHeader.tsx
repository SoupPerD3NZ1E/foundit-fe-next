...
import {
  House, Search, SquarePlus, BriefcaseBusiness,
  MessageSquareMore, Bell, ChevronDown, ChevronRight,
...
const navItems: { key: MenuToggle; label: string; icon: any; href: string }[] = [
  { key: 'dashboard', label: 'Dashboard', icon: House, href: '/client/dashboard' },
  { key: 'find-freelancers', label: 'Find Freelancers', icon: Search, href: '/client/browse-freelancers' },
  { key: 'browse-gigs', label: 'Browse Gigs', icon: SquarePlus, href: '/client/browse-gigs' },
  { key: 'orders', label: 'My Orders', icon: BriefcaseBusiness, href: '/client/my-orders' },
];
...
export default function ClientHeader() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const name = payload.fullName ?? payload.name ?? payload.preferred_username ?? payload.sub ?? 'Client';
      setDisplayName(name);
  ...
  return (
    <header className="mx-auto flex w-full h-16 border-b border-b-gray-300 justify-between items-center px-6 py-4 lg:px-10 sticky top-0 z-50 bg-white">
      ...
      <div className="flex items-center gap-8">
        <nav className="hidden items-center gap-2 lg:flex">
          {navItems.map(({ key, label, icon: Icon, href }) => (
            <Link key={key} href={href} className={navClass(key)}>
      ...
      <div className="flex items-center gap-5">
        ...
        <Link href="/client/chat" className="relative inline-flex h-10 w-10 items-center justify-center rounded-full text-[#6b7280] transition hover:bg-[#f9fafb] hover:text-[#111827]">
          <MessageSquareMore className="h-5 w-5" />
        </Link>

        <Link href="/client/notification" className="relative inline-flex h-10 w-10 items-center justify-center rounded-full text-[#6b7280] transition hover:bg-[#f9fafb] hover:text-[#111827]">
        ...
        <div className="relative" ref={panelRef}>
          ...
          >
            <div className="h-10 w-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-sm font-semibold text-gray-500 shadow-sm shrink-0">
              {initials}
            </div>
            <div className="hidden text-left sm:block pr-1">
              ...
              <p className="text-sm leading-5 text-[#6b7280]">Client</p>
            </div>
          ...
          <div className={`absolute top-[calc(100%+10px)] right-0 w-72 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50 transition-all duration-200 origin-top-right ${panelOpen ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'}`}>
            <div className="relative overflow-hidden bg-gradient-to-br from-slate-800 to-slate-600 px-5 pt-5 pb-6">
              ...
              <div className="relative flex items-center gap-3">
                <div className="h-14 w-14 rounded-full bg-white border border-gray-200 flex items-center justify-center text-xl font-semibold text-gray-500 shrink-0">
                  {initials}
                </div>
                <div>
                  <p className="text-[15px] font-semibold text-white leading-5">{displayName}</p>
                  <p className="text-sm text-slate-300 mt-0.5">Client</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-2">
              {[
                { key: 'profile', label: 'My Profile', desc: 'View & edit your details', icon: User, href: '/client/my-profile' },
                { key: 'settings', label: 'Settings', desc: 'Preferences & security', icon: Settings, href: '/client/setting' },
                { key: 'ekyc', label: 'eKYC', desc: 'Identity verification', icon: CreditCard, href: '/index/ekyc' },
              ].map(({ key, label, desc, icon: Icon, href }) => (
                <Link key={key} href={href} onClick={() => setPanelOpen(false)}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors hover:bg-gray-50">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gray-100 text-gray-500 group-hover:bg-gray-200">
                    <Icon className="h-5 w-5" />
                  </span>
                  <div className="flex-1 min-w-0 text-left">
                    <p className="text-[14px] font-semibold text-gray-800 leading-5">{label}</p>
            ...
            <div className="px-2 pb-3">
              ...
              <button type="button" onClick={logout}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors hover:bg-rose-50">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-rose-50 text-rose-500">
                  <LogOut className="h-5 w-5" />
                </span>
                <div className="flex-1 text-left">
                  <p className="text-[14px] font-semibold text-rose-500 leading-5">Logout</p>
                  <p className="text-[12px] text-rose-300 mt-0.5 leading-4">End your session</p>
                </div>
              </button>
            </div>
          </div>
